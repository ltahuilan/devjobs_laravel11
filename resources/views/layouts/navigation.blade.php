<nav x-data="{ open: false }" class="bg-neutral-100 dark:bg-neutral-800 border-b border-neutral-100 dark:border-neutral-700">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <!-- Navigation Links -->
                {{-- Puede ver si esta autenticado --}}
                @auth
                    {{-- PUEDE VER SOLO SI ES RECLUTADOR --}}
                    @can('viewAny', App\Model\Vacancy::class)
                    ...
                    {{-- SI ES RECLUTADOR NO PUEDE VER --}}
                    @cannot('viewAny', App\Model\Vacancy::class)
                        
                        @if(!request()->routeIs('home'))                            
                            <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                                <x-nav-link :href="route('home')" :active="request()->routeIs('vacancies.index')">
                                    {{ __('See all vacancies') }}
                                </x-nav-link>
                            </div>
                        @endif
                    @endcannot
            ...
            @auth
                <div class="hidden sm:flex sm:items-center sm:ms-6">
                    ...
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-neutral-500 dark:text-neutral-100 bg-neutral-100 dark:bg-neutral-800 hover:text-neutral-700 dark:hover:text-neutral-300 focus:outline-none transition ease-in-out duration-150">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        @auth
            ...
            <div class="p-2 border-t border-neutral-200 dark:border-neutral-600">
                ...
                @if( auth()->user()->rol === 1)
                    <div class="flex gap-3 items-center my-4 text-white">
                        ...
                        <p class="text-base font-medium text-neutral-600 dark:text-neutral-400">Notifications</p>
                    </div>
                @endif
        ...
        <!-- Responsive Navigation Menu for guest-->
        @guest
            <div class="pt-2 pb-3 space-y-1">
